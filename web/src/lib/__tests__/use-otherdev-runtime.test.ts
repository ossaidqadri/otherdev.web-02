...
describe("attachment content to Groq API format mapping", () => {
  it("maps image content to image_url format", () => {
    const result = mapAttachmentContentToGroq([{ type: "image", image: "data:image/png;base64,abc" }]);
...
describe("extractAttachmentData", () => {
  it("sets hasImages true when image content present", () => {
    const { hasImages } = extractAttachmentData([{ content: [{ type: "image", image: "data:image/png;base64,x" }] }]);
  ...
  it("extracts imageUrls as { url } objects", () => {
    const { imageUrls } = extractAttachmentData([{ content: [{ type: "image", image: "data:image/jpeg;base64,y" }] }]);
